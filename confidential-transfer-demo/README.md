...
This is a companion to, and deliberately kept in its own subfolder separate from,
[`../sales-demo`](../sales-demo) — that one is an illustrative/mocked walkthrough of the
private-channel + confidential-balances architecture for a business audience. This one is
the technical proof: real keys, a real mint, a real transfer, verifiable independently on
Solana's devnet explorer.
...
secrets, no environment variables). Short version: point your host at the repo root with
`confidential-transfer-demo/ui` as the root/subdirectory and build command `npm run build`.

## Related demo

[`../sales-demo`](../sales-demo) — the mocked/illustrative sales walkthrough this build is
a technical companion to.

## Why a separate subfolder, not blended together

The sales demo intentionally uses mocked/simulated data — it's built to run instantly in
a browser for a live pitch, and private channels themselves are a pre-audit reference
implementation with no public devnet deployment to point at. This one is the opposite
tradeoff: it's slower to run and requires devnet SOL, but every number in it is real,
because Token-2022 Confidential Balances **is** live on devnet (and mainnet) today.
Keeping them in separate subfolders (rather than one blended app) avoids implying the
sales-demo walkthrough is backed by the same kind of on-chain proof this one provides.
